...
const required = [
  ...
  'docs/INDEX.md', 'docs/ARCHITECTURE.md', 'docs/CONFIGURATION.md', 'docs/PROVIDERS.md',
...
for (const relative of required) {
  if (!(await stat(path.join(root, relative)).catch(() => null))) throw new Error(`Missing required public file: ${relative}`);
}
for (const relative of forbiddenRoots) {
  if (await stat(path.join(root, relative)).catch(() => null)) throw new Error(`Legacy or staging root must not be published: ${relative}`);
}
...
async function validateJsx(files) {
  let typescript;
  try { typescript = await import('typescript'); } catch { return; }
  for (const file of files) {
    const source = await readFile(file, 'utf8');
    const result = typescript.transpileModule(source, {
      fileName: file,
      reportDiagnostics: true,
      compilerOptions: { jsx: typescript.JsxEmit.ReactJSX, target: typescript.ScriptTarget.ES2022, module: typescript.ModuleKind.ESNext },
    });
    const errors = (result.diagnostics ?? []).filter((diagnostic) => diagnostic.category === typescript.DiagnosticCategory.Error);
    if (errors.length) throw new Error(`${relative(file)} failed JSX validation: ${errors.map((item) => typescript.flattenDiagnosticMessageText(item.messageText, '\n')).join('; ')}`);
  }
}
...
async function validateStaleReferences(files) {
  ...
  for (const file of files.filter((item) => /\.(?:md|json|js|mjs|cjs|jsx|html|css|yml|yaml)$/.test(item))) {
    ...
    const source = await readFile(file, 'utf8');
    for (const term of forbidden) if (source.toLowerCase().includes(term.toLowerCase())) throw new Error(`Stale legacy reference ${JSON.stringify(term)} found in ${relative(file)}.`);
  }
}

async function validatePackage() {
  const pkg = JSON.parse(await readFile(path.join(root, 'package.json'), 'utf8'));
  if (pkg.name !== 'subscription-proxy-inator' || pkg.version !== '2.0.0') throw new Error('Package identity must be subscription-proxy-inator@2.0.0.');
...
async function validateRendererBoundary() {
  ...
  if (exposed.length < 10 || exposed.length > 40 || new Set(exposed).size !== exposed.length) throw new Error('Preload bridge must expose a finite, unique capability set.');
  const renderer = (await Promise.all((await walk(path.join(root, 'desktop/renderer'))).map((file) => readFile(file, 'utf8')))).join('\n');
  if (/secretRef|apiKeySecretRef|ipcRenderer|require\s*\(/.test(renderer)) throw new Error('Renderer contains a forbidden secret reference or direct process capability.');
  const main = await readFile(path.join(root, 'desktop/main/index.js'), 'utf8');
  if (!/contextIsolation:\s*true/.test(main) || !/sandbox:\s*true/.test(main) || !/nodeIntegration:\s*false/.test(main)) throw new Error('BrowserWindow must keep context isolation and sandboxing enabled with Node integration disabled.');
}
...
async function validateWorkflowActions(files) {
  ...
  for (const file of workflows) {
    const source = await readFile(file, 'utf8');
    if (/actions\/checkout@v[1-5]\b/.test(source) || /actions\/setup-node@v[1-5]\b/.test(source)) throw new Error(`${relative(file)} uses an obsolete Node-action generation.`);
  }
}
