...
const navigation = await Promise.all(
  markdownFiles
    ...
    .map(async (file) => ({
      ...
      title: titleOf(await readFile(file, 'utf8')),
    })),
);
...
for (const sourcePath of markdownFiles) {
  ...
  const nav = navigation.map((item) => {
    const target = item.relative === 'INDEX.md'
      ? path.join(output, 'docs')
      : path.join(output, 'docs', item.relative.replace(/\.md$/, '').toLowerCase());
...
<html lang="en">
  <head>
    ...
    <title>${escapeHtml(title)} · Subscription Proxy Inator</title>
  ...
  <body class="docs-page">
    <a class="skip-link" href="#documentation">Skip to content</a>
    <header class="site-header">
      <div class="site-header-inner">
        ...
        <nav id="site-navigation" class="site-navigation" aria-label="Primary navigation" data-menu>
          <a href="${rootPrefix}/#capabilities">Capabilities</a>
          <a href="${rootPrefix}/docs/">Documentation</a>
          <a href="https://github.com/surajmandalcell/subscription-proxy-inator">GitHub</a>
        </nav>
      </div>
    </header>
    <div class="docs-layout">
      <aside class="docs-nav" aria-label="Documentation navigation">${nav}</aside>
      <main class="docs-content" id="documentation"><article>${body}</article></main>
    </div>
  </body>
</html>`;
...
const sitemap = [
  ...
  ...markdownFiles
    ...
    .map((file) => `docs/${path.relative(docsRoot, file).replace(/\.md$/i, '').toLowerCase()}/`),
]
  .map((entry) => `<url><loc>https://surajmandalcell.github.io/subscription-proxy-inator/${entry}</loc></url>`)
  .join('');
...
async function copyWebsiteFonts() {
...
function titleOf(source) {
  return /^#\s+(.+)$/m.exec(source)?.[1]?.replace(/[`*_]/g, '') ?? 'Documentation';
}
...
function renderMarkdown(source, sourcePath, outputDirectory) {
  ...
  const flushParagraph = () => {
    ...
    html.push(`<p>${inline(paragraph.join(' '), sourcePath, outputDirectory)}</p>`);
    paragraph = [];
  };
  const flushList = () => {
    if (!list) return;
  ...
  while (index < lines.length) {
    ...
    if (heading) {
      ...
      html.push(`<h${level} id="${slug(text)}">${inline(text, sourcePath, outputDirectory)}</h${level}>`);
      index += 1;
      continue;
    }
    ...
    if (line.startsWith('> ')) {
      flushParagraph();
      flushList();
      html.push(`<blockquote>${inline(line.slice(2), sourcePath, outputDirectory)}</blockquote>`);
      index += 1;
      continue;
    }
    ...
    if (/^\|.+\|$/.test(line) && /^\|?\s*:?-+/.test(lines[index + 1] ?? '')) {
      ...
      html.push(`<div class="doc-table"><table><thead><tr>${headers.map((cell) => `<th>${inline(cell, sourcePath, outputDirectory)}</th>`).join('')}</tr></thead><tbody>${rows.map((row) => `<tr>${row.map((cell) => `<td>${inline(cell, sourcePath, outputDirectory)}</td>`).join('')}</tr>`).join('')}</tbody></table></div>`);
      continue;
    }
    ...
    if (unordered || ordered) {
      ...
      html.push(`<li>${inline((unordered ?? ordered)[1], sourcePath, outputDirectory)}</li>`);
      index += 1;
      continue;
    }
...
function inline(value, sourcePath, outputDirectory) {
  ...
  text = text.replace(
    /\[([^\]]+)\]\(([^)\s]+)(?:\s+&quot;[^&]*&quot;)?\)/g,
    (_match, label, href) => `<a href="${escapeHtml(rewriteHref(href, sourcePath, outputDirectory))}">${label}</a>`,
  );
  return text;
}
...
function rewriteHref(href, sourcePath, outputDirectory) {
  ...
  const targetOutput = relative === 'INDEX'
    ? path.join(output, 'docs')
    : path.join(output, 'docs', relative.toLowerCase());
