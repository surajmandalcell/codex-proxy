...
test('public text uses the ASD-STE100 project gate', async () => {
  ...
  assert.match(checker, /desktop\/renderer/);
  assert.match(checker, /javascriptSegments/);
  assert.match(checker, /ts\.isJsxText/);
  assert.match(checker, /file\.endsWith\('\.svg'\)/);
  assert.match(checker, /path\.join\(root, '\.github'\)/);
  assert.match(checker, /const namedEntities = new Map/);
  assert.match(checker, /function stripHtmlElement/);
  assert.match(checker, /function containsPhrase/);
  assert.match(checker, /segment\.instruction \? 20 : 25/);
  assert.doesNotMatch(checker, /lower\.includes\(phrase\)/);
